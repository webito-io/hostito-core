...
export class ProvisionersWorker extends WorkerHost {
    ...
    async process(job: Job<ProvisioningJobData>): Promise<any> {
        ...
        if (!server || !server.provisioner.isActive) throw new Error('Provisioner missing or inactive');

        // 3. Execute
        const provider = this.provisionersFactory.get(server.provisioner.name as ProvisionerType);

        if (typeof provider[actionName] !== 'function') {
            throw new Error(`Action ${actionName} unsupported by provider ${server.provisioner.name}`);
        }
