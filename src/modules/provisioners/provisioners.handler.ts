...
import { ProvisionerProvider, ProvisionerType } from './providers/provisioners.provider.interface';
...
export class ProvisionersHandler {
    ...
    async executeAction(serviceId: number, actionName: string, extraArgs: Record<string, unknown> = {}) {
        try {
            await this.provisionersQueue.add('execute-action', {
                serviceId,
                actionName,
                extraArgs,
            }, {
            ...
            return { status: 'pending', message: `Action ${actionName} added to provisioners queue` };
        } catch (error: any) {
    ...
    async testConnection(serverId: number) {
        ...
        const provider: ProvisionerProvider = this.provisionersFactory.get(server.provisioner.name as ProvisionerType);
