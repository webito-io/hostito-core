...
export class SpaceshipProvider implements DomainProvider {
  ...
  private async request<T = any>(
    ...
    if (!res.ok) {
      ...
      throw new Error(`Spaceship API [${res.status}]: ${err.detail}`);
    }
  ...
  async availability(domainName: string, registrar: Registrar): Promise<boolean> {
    const config = this.getConfig(registrar);
    const data = await this.request<{ result: string }>(config, 'GET', `/domains/${domainName}/available`);
  ...
  async register(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    ...
    const { domain, contacts, years = 1, autoRenew = false, privacyProtection } = args;
    try {
      await this.request(config, 'POST', `/domains/${domain.name}`, {
        autoRenew,
        years,
        privacyProtection: privacyProtection ?? { level: 'high', userConsent: true },
        contacts,
      });
      return { status: 'success', message: `Domain ${domain.name} registration initiated` };
    } catch (e: any) {
  ...
  async renew(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    ...
    try {
      await this.request(config, 'POST', `/domains/${domain.name}/renew`, {
        years,
        currentExpirationDate: currentExpirationDate ?? domain.expiresAt?.toISOString(),
      });
      return { status: 'success', message: `Domain ${domain.name} renewal initiated` };
    } catch (e: any) {
      return { status: 'failed', message: e.message };
    }
  }
  ...
  async lock(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    const config = this.getConfig(args.registrar);
    try {
      await this.request(config, 'PUT', `/domains/${args.domain.name}/transfer/lock`, { isLocked: true });
      return { status: 'success', message: `Domain ${args.domain.name} locked` };
    } catch (e: any) {
      return { status: 'failed', message: e.message };
    }
  }
  ...
  async unlock(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    const config = this.getConfig(args.registrar);
    try {
      await this.request(config, 'PUT', `/domains/${args.domain.name}/transfer/lock`, { isLocked: false });
      return { status: 'success', message: `Domain ${args.domain.name} unlocked` };
    } catch (e: any) {
      return { status: 'failed', message: e.message };
    }
  }
  ...
  async code(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    ...
    try {
      const data = await this.request<{ authCode: string; expires: string }>(
        config, 'GET', `/domains/${args.domain.name}/transfer/auth-code`,
      );
  ...
  async nameservers(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    const config = this.getConfig(args.registrar);
    try {
      const data = await this.request(config, 'PUT', `/domains/${args.domain.name}/nameservers`, {
        provider: 'custom',
  ...
  async privacy(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    const config = this.getConfig(args.registrar);
    try {
      await this.request(config, 'PUT', `/domains/${args.domain.name}/privacy/preference`, {
        privacyLevel: args.enabled ? 'high' : 'public',
        userConsent: true,
      });
      return { status: 'success', message: `Domain ${args.domain.name} privacy updated` };
    } catch (e: any) {
      return { status: 'failed', message: e.message };
    }
  }
  ...
  async transfer(args: DomainProvisioningArgs): Promise<DomainProvisionResult> {
    const config = this.getConfig(args.registrar);
    const { domain, contacts, authCode, autoRenew = false, privacyProtection } = args;
    try {
      ...
      return { status: 'success', message: `Domain ${domain.name} transfer initiated` };
    } catch (e: any) {
      return { status: 'failed', message: e.message };
    }
  }
