...
export class NotificationsWorker extends WorkerHost {
    ...
    async process(job: Job<NotificationDto>): Promise<any> {
        ...
        try {
            ...
            const driver = drivers.value && (drivers.value as Record<string, string>)[notification.type];
            if (!driver) {
                throw new Error(`Notification driver not found for type: ${notification.type}`);
            }
            ...
            if (!template) {
                throw new Error(`Notification template '${notification.template}' not found`);
            }
